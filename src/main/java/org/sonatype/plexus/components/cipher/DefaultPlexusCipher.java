    ...
    implements PlexusCipher, Initializable
{
      ...
      throws PlexusCipherException
  {
    ...
    try
    {
      ...
      SecretKey key = SecretKeyFactory.getInstance( algorithm,
          SECURITY_PROVIDER ).generateSecret( keySpec );
      ...
      throws PlexusCipherException
  {
    ...
    try
    {
      ...
      if( saltLen != SALT_SIZE )
        throw new Exception(
            "default.plexus.cipher.encryptedStringCorruptedStructure" );

      if( res.length < (saltLen + 2) )
        throw new Exception(
            "default.plexus.cipher.encryptedStringCorruptedLength" );
      ...
      if( decLen < 1 )
        throw new Exception(
            "default.plexus.cipher.encryptedStringCorruptedSize" );
  ...
  // ---------------------------------------------------------------
  public String decryptDecorated(
      String str,
      String passPhrase )
      throws PlexusCipherException
  {
  ...
  {
    if( !isEncryptedString( str ) )
      throw new PlexusCipherException(
          "default.plexus.cipher.badEncryptedPassword" );
  ...
  public String decorate(
      String str )
  {
    return ENCRYPTED_STRING_DECORATION_START + (str == null ? "" : str)
      ...
      for( String serviceType : serviceTypes )
      {
        ...
        else
        {
          System.out.println( serviceType
              + ": does not have any providers in this environment" );
        }
      }
  }
  // ---------------------------------------------------------------
